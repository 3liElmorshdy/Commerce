...
function Products() {
 ...
 const [state, setState] = useState([]);
  ...
  // useEffect(function () {
  //   axios.get("https://dummyjson.com/https://dummyjson.com").then((res) => {
  ...
  useEffect(()=>{
    myAxios.get("/products").then((res)=>{setState(res.data.products)})
  },[])


  const SerchBarFilter = state.filter((e) =>
    e.title.toLowerCase().includes(serchBar.toLowerCase())
  );
  ...
  return (
          ...
          <input type="text" name="searhBar" id="search" placeholder="input searchBar"  className="form-control" onChange={((e)=>setSearchBar(e.target.value))  } />


        <div className="container">
          <div className="row">
            {SerchBarFilter.map((product) => (
              <ProductCard
                key={product.id}
                data={product}        
              />
...
//           <h2>First Product:</h2>
//             {console.log(`this s the {s}`, state)}
// {/*             
//             state.length !== 0 ? (
//               <>
//                 <p>{state[0].title}</p>
